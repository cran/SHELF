...
We can use the function `elicitConcProb` to open a browser (using `shiny`) in which we can adjust the concordance probability and view a corresponding joint sample. 
...
Continuing the example, we elicit a marginal distribution for $X_3$ (details are not important).
...
theta <- copulaSample(myfit1, myfit2, myfit3, 
                      ...
                      d = rep("Beta", 3))
```
The output tells us how any one of the concordance probabilities should be adjusted to make it cohere with the other two. For example, $p_{2,3}$ would need to be specified in the range $[0.5, 0.9]$, given the values of $p_{1,2}$ and $p_{1,3}$. To confirm this
```{r}
conc.prob[2, 3] <- 0.9
theta <- copulaSample(myfit1, myfit2, myfit3, 
                      cp = conc.prob, n = 1000, 
